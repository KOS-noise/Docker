...
### 문제 4 — docker-compose web, backend, db 환경 구축

React로 작성한 프론트엔드와 Spring Boot 백엔드, MySQL DB를 `docker compose`로 함께 올리고, 프론트에서 백엔드 API를 호출하는 환경을 구성하세요.

조건:

- 프론트 코드를 React로 구성한 뒤, 백엔드의 `/api/cors/greeting` 을 호출하여 응답을 화면에 표시함
- `docker compose` 에서 `networks` 를 사용해 **web(프론트)**, **backend**, **db** 역할이 드러나도록 서비스를 나누고, **프론트 컨테이너는 DB 컨테이너에 네트워크상으로 접근할 수 없도록** 설정함 (예: DB는 내부 전용 네트워크에만 두고, 프론트는 해당 네트워크에 연결하지 않음)

> 힌트: 백엔드는 DB가 있는 네트워크와 프론트가 있는 네트워크 둘 다에 붙여야 하며, 브라우저에서 호스트 포트로 DB에 붙는 것과 컨테이너 간 격리는 별개임을 구분할 것.
